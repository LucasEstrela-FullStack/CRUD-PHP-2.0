...
$sql = "SELECT * FROM `cadastro` WHERE userId = $userId";
...
<head>
    ...
    <meta
        name="viewport"
        content="wuserIdth=device-wuserIdth, initial-scale=1, shrink-to-fit=no" />
...
<body>
    ...
    <a class="btn btn-sm btn-danger mt-2" href="/">Sair</a>
        ...
        <form class="form d-flex gap-3 mt-3" action="atualizar.php?userId=<?=$result['userId']?>" method="post">
            ...
            <input value="<?=$result['dataNAS']?>" autocomplete="off" class="form-control" type="date" name="dataNAS">
